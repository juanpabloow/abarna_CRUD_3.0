...
import { updateCiudadModal, deleteCiudadModal } from '@/app/actions/modal-actions'
import { Trash2 } from 'lucide-react'

export function CiudadEditForm({ ciudad, onSuccess }: { ciudad: any; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true)
    try {
      await updateCiudadModal(ciudad.ciudad_id, new FormData(e.currentTarget))
      router.refresh()
      onSuccess()
    } finally {
      setLoading(false)
  ...
  const handleDelete = async () => {
    setLoading(true)
    try {
      await deleteCiudadModal(ciudad.ciudad_id)
      router.refresh()
      onSuccess()
    } finally {
      setLoading(false)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        ...
        <Input id="ciudad" name="ciudad" required defaultValue={ciudad.ciudad} />
      </div>
